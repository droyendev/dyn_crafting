...
$(document).ready(function () {
  window.addEventListener("message", function (event) {
    ...
    switch (event["data"]["openMenu"]) {
      case true:
        ...
        $.post(
        "http://dyn_crafting/updateInventory",
        JSON.stringify({}),
        (data) => {
          for (let x = 1; x <= 100; x++) {
            ...
            $(`#${i}`).droppable({
              ...
              drop: function (event, ui) {
                if ($(`#${i}`).is(":empty")) {
                  ...
                  $(".popUp").fadeIn();
                  $(`#${i}`).append(
                    `<img src="${image}" class="craft-imageWaiting" alt="">`
                  );
                  boxColocada = `#${i}`;
                  $("#buttonPopUp")
                    .unbind()
                    .click(function (event) {
                      ...
                      if (valorInserido <= valorPadrao) {
                        if( valorInserido > 0 ){
                          ...
                          $(`${boxColocada}`).append(
                            `<p class="craft-amount">x${valorInserido}</p>`
                          );
                          $(".popUp").hide();
                          ...
                          if (valorPadrao === valorInserido) {
                            ...
                            parent.children[2].textContent = `x${
                              valorPadrao - valorInserido
                            }`;
                          }
                        } else {
                          $(`${boxColocada}`).html("");
                        ...
                        $(".error").show();
                        $(".error").html(
                          "A quantia inserida no Craft é maior que a quantia da sua mochila!"
                        );
                        $(".box").css("background", "rgba(11, 16, 20, 0.7)");
                        setTimeout(() => {
                          $(".error").hide();
  ...
  $("#craft-button").unbind().click(function (event) {
    this.disabled = true
    parent = this.parentNode
    for (let i = 0; i <= 8; i++) {
      if( !$(`#${i+1}`).is(':empty') ){
